<template>
  <div class="recommend" v-if="data?.options">
    <div class="recommend-left">
      <div class="head-recommend" :style="{ background: msgLeft.bgColor }">
        <span>{{ msgLeft.title }}</span>
        <span class="hover-pointer" @click="linkTo(msgLeft.url)">
          {{ msgLeft.secondTitle }}&gt;
        </span>
      </div>
      <div class="content-left">
        <div>
          ...
          <el-button
            size="small"
            :style="{ background: msgLeft.bgColor, color: '#fff' }"
            @click="linkTo(msgLeft.list[0].url)"
...
const msgLeft = computed(() => props.data?.options?.contentLeft || { list: [] })
