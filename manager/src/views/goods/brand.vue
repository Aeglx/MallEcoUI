        ...
        <el-form-item label="品牌图标" prop="logo">
          <div class="logo-container">
            <el-image
              ...
              fit="contain"
              style="width: 120px; height: 90px; border: 1px solid #dcdee2"
            />
...
import { getBrandList, addBrand, updateBrand, deleteBrand } from '@/api/goods'

const defaultImage = '/default-brand.png'
  ...
  await formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      try {
        let res
        if (form.id) {
          res = await updateBrand(form.id, form)
        } else {
          res = await addBrand(form)
        }
        if (res.success) {
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确认删除此品牌？', '提示', {
