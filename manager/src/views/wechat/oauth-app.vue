  ...
  <div class="oauth-app-page">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="appId" label="AppID" width="200" show-overflow-tooltip />
...
import { getWechatOAuthAppList } from '@/api/wechat'
...
const getData = async () => {
  ...
  try {
    ...
    const res = await getWechatOAuthAppList(params)
    if (res.success) {
      tableData.value = res.result?.records || res.result?.list || res.result || []
      pagination.total = res.result?.total || 0
    }
  } catch (error) {
    console.error('获取OAuth应用列表失败:', error)
  } finally {
    loading.value = false
  }
}
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确认删除此应用？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      // const res = await deleteOAuthApp(row.id)
      // if (res.success) {
