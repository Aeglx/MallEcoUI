...
const getData = async () => {
  ...
  try {
    ...
    const res = await getWechatMaterialList('video', params)
    if (res.success) {
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确认删除此视频？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      const res = await deleteWechatMaterial('video', row.id.toString())
      if (res.success) {
