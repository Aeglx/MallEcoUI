    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="refundMethod" label="退款方式" width="120">
          <template #default="{ row }">
            <el-tag v-if="row.refundMethod === 'ORIGINAL'" type="info">原路退回</el-tag>
            <el-tag v-else type="warning">线下退款</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="退款状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.status === 'SUCCESS'" type="success">退款成功</el-tag>
            <el-tag v-else-if="row.status === 'PROCESSING'" type="warning">退款中</el-tag>
            <el-tag v-else-if="row.status === 'PENDING'" type="info">待退款</el-tag>
...
import { getRefundFlowList } from '@/api/after-sale'
...
const getData = async () => {
  ...
  try {
    ...
    const res = await getRefundFlowList(params)
    if (res.success) {
      tableData.value = res.result?.records || res.result?.list || res.result || []
      pagination.total = res.result?.total || 0
    }
  } catch (error) {
    console.error('获取退款流水失败:', error)
  } finally {
    loading.value = false
  }
}
