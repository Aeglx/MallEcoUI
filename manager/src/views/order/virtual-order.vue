  ...
  <div class="virtual-order-page">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="goodsName" label="虚拟商品" min-width="200" />
...
import { getVirtualOrderList } from '@/api/order'
...
const getData = async () => {
  ...
  try {
    ...
    const res = await getVirtualOrderList(params)
    if (res.success) {
      tableData.value = res.result?.records || res.result?.list || res.result || []
      pagination.total = res.result?.total || 0
    }
  } catch (error) {
    console.error('获取虚拟订单列表失败:', error)
  } finally {
    loading.value = false
  }
}
